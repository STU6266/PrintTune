...
describe("PrintTune SQLite database", () => {
  ...
  it("applies all migrations and records schema version 8", () => {
    ...
    try {
      ...
      expect(readSchemaVersion(inspectionDatabase)).toBe(10);
    } finally {
  ...
  it("preserves the migrated schema after a file-backed database is reopened", () => {
    ...
    try {
      expect(secondConnection.schemaVersion()).toBe(10);
  ...
  it("preserves the current schema and existing data across reopen and remains idempotent", () => {
    ...
    try {
      ...
      expect(readSchemaVersion(secondDatabase)).toBe(10);
