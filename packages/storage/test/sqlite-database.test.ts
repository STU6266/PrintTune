...
describe("PrintTune SQLite database", () => {
  ...
  it("applies all migrations and records schema version 7", () => {
    ...
    try {
      ...
      expect(readSchemaVersion(inspectionDatabase)).toBe(7);
    } finally {
  ...
  it("preserves the migrated schema after a file-backed database is reopened", () => {
    ...
    try {
      expect(secondConnection.schemaVersion()).toBe(7);
  ...
  it("preserves the current schema and existing data across reopen and remains idempotent", () => {
    ...
    try {
      ...
      expect(readSchemaVersion(secondDatabase)).toBe(7);
