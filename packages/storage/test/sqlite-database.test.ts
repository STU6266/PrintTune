...
describe("PrintTune SQLite database", () => {
  ...
  it("applies all migrations and records schema version 6", () => {
    ...
    try {
      ...
      expect(readSchemaVersion(inspectionDatabase)).toBe(6);
    } finally {
  ...
  it("preserves the migrated schema after a file-backed database is reopened", () => {
    ...
    try {
      expect(secondConnection.schemaVersion()).toBe(6);
  ...
  it("preserves the current schema and existing data across reopen and remains idempotent", () => {
    ...
    try {
      ...
      expect(readSchemaVersion(secondDatabase)).toBe(6);
